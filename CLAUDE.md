...
2. **Library** (`/home`) — document list left, document detail right. ContextBar: Rename · Duplicate · Delete · Open · Import · New Document. Rename/Duplicate/Delete/Open are disabled when no document is selected. Open is primary when a document is selected; New Document is primary otherwise. Delete sets `pendingDelete` (shows inline confirmation bar below the ContextBar); the other actions fire directly. Import triggers a hidden `.docx` file input. DOCUMENT DETAIL panel header is label-only — no buttons. Document title displayed as `text-lg font-semibold` (rename input matches). Duplicate calls `POST /documents/{doc_id}/duplicate`, prepends the new doc to the list, and selects it (no navigation). Import error bar also surfaces duplicate errors.
