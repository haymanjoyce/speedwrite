...
- **Sectional editing (Document chat)**: AI proposes changes only when the user attaches a section via the `+` picker. No attachment → chat-only, no diff view. The AI returns only the rewritten section in `<proposed_section>` tags; `chat.py` splices it back into the full document and returns the result as `proposed_content`. Diff/Accept/Reject runs on (original full doc, spliced doc) unchanged.
  ...
  - `ignore_history: true` always (both section-attached and chat-only). Chat-only was changed from `false` because prior rewrite turns in history cause Haiku to hallucinate edit summaries in chat-only mode. If hallucination still occurs, the user can clear chat history via the trash icon to reset context.
...
- **LLM abstraction** (`llm.py`): Anthropic is the only active path; `_complete_ollama()` is retained but dormant. Active model: `FREE_MODEL = "claude-haiku-4-5-20251001"`. `_complete_anthropic` catches status 529 → HTTP 503 with a user-facing message. It extracts the reply by joining all `type == "text"` blocks in `response.content` (not `content[0].text`) — Sonnet with default `effort=high` returns a thinking block at position 0 before the text; thinking content is never returned.
- **Model ID convention**: Anthropic model IDs from the 4.6 generation onward are dateless pinned snapshots (e.g. `claude-sonnet-5`, `claude-opus-4-8`). Do not "correct" a dateless ID by adding a `-YYYYMMDD` suffix — it breaks routing. Older IDs (Haiku 4.5 and earlier, e.g. `claude-haiku-4-5-20251001`) still carry date suffixes; the two conventions coexist.
...
- **Single Send button**: no Edit/Chat split. Mode is signalled by `section_path` — `null` = chat-only (AI cannot edit the document; if user sends an edit-shaped instruction, AI tells them to attach a section via `+` and resend); otherwise rewrite mode.
