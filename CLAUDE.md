...
**Tier 1 — Global bar (TopBar):** Always visible. Background is near-black (`bg-gray-900`); all interior text and icons are light (`text-white`, `text-gray-300`, `text-gray-400`). The bottom border is removed. Props: `user`, `onLogout`, `docTitle`, `isRenaming`, `onRenameSave`, `onRenameCancel`, `pageTitle`, `onFeedbackClick`, `showBack`. SpeedWrite logo links to `/home`. When `showBack={true}` (Document, Evidence, History, Images, Account, Admin), a `←` ghost-style icon button renders instead of the logo and navigates to `/home` on click — renders immediately on mount without waiting for `docTitle` to load. Rename input border is `border-gray-500` (light gray, visible on dark). User dropdown: "Give feedback" (when `onFeedbackClick` provided) · "Administration" (admins only) · "Account settings" · "Sign out"; closes on outside click or Escape. The dropdown menu itself is white with dark text. `pageTitle` is for non-document pages; `docTitle` takes priority if both are set.

**Tier 2 — Page context bar (ContextBar):** Below the global bar. Left side: tab navigation (Document / Evidence / History); active tab has a 2px bottom border in near-black (`border-b-2 border-gray-900`), flush with the bottom of the bar — all tabs carry a transparent `border-b-2` by default to prevent layout shift; inactive tabs get `hover:border-gray-300` on hover. Right side: page-specific action buttons (outlined). ContextBar accepts a `tabs` prop: `[{ label, active, onClick }]`. Action objects support `disabled: true`. The optional `controls` prop renders between the tabs and the actions group. The optional `rightControls` prop renders inside the actions flex row, to the left of the action buttons — use this for dropdowns that must sit alongside action buttons. Per-page action inventories are in App Architecture below.
...
- Delete buttons in the ContextBar use variant: 'default' when an inline confirmation bar follows; danger variant only when there is no separate confirmation step. Primary actions (Add, New) always near-black
...
1. **Landing** (`/`) — public, unauthenticated, no TopBar/ContextBar, no nav, no footer. Full-height white page (`h-screen bg-white`), all content vertically and horizontally centred. Content (top to bottom): "SpeedWrite" brand name (`text-sm font-semibold tracking-widest uppercase text-gray-400`) · "Write documents faster." headline (`text-3xl font-semibold`, Lora serif via Google Fonts `@import`, equidistant between brand name and button) · "Create account" primary `Button` linking to `/register` (padding overridden via inline style to `paddingLeft/Right: 4rem`) · "Sign in" link to `/login` (`text-sm text-blue-600 underline`). Logout and account-delete both redirect here. File: `LandingPage.jsx`.
