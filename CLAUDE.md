...
│       │   ├── ActionsDropdown.jsx   # Reusable dropdown pill — accepts title + actions[] props; used for Redraft and Insights
...
- **Document actions**: Whole-document AI actions split into two dropdowns. **Redraft** actions (Rewrite, Restructure, Expand, Condense, Simplify, Formalise) show an InstructionBar for optional instructions, then fire via `chatPanelRef.current.fireInsight(prompt)` — routed through the chat panel so they benefit from the Stop button, chat history, and consistent UX. **Insights** actions (Summarise, Extract key points, Critique, Suggest improvements) fire immediately via `fireInsight`. Both use `REDRAFT_PROMPTS`/`INSIGHTS_PROMPTS` string maps in `Document.jsx` — no call to `backend/actions.py` from these paths. `ActionsDropdown.jsx` is a reusable component accepting `title` and `actions` props. `Home.jsx` description generation still calls `api.documentAction` directly (summarise action).
