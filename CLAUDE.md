...
- **Evidence chat** (`EvidenceChatPanel.jsx`): Persistent chat on Evidence page. Never modifies the document. When no source is manually attached, `_build_evidence_block` (from `chat.py`) auto-injects the full evidence base — same RAG/full-dump logic as document chat; inactive sources excluded. A source inventory is always prepended so the LLM can report what sources exist. Attached-context and auto-evidence-block paths are mutually exclusive. `DELETE .../evidence-chat` clears history; "Clear chat" resets messages and `sentFingerprintRef`. No "All sources" option in `SourcePickerPopup`. **Stale-context warning**: `sentFingerprintRef` tracks active-source fingerprint at send time; if it differs (sources toggled), the trash icon highlights and tooltip prompts clearing.
...
- **Gutter** (`ResizableGutter.jsx`): `side` prop `'left'` or `'right'` determines delta direction. `onResize(newWidth)` fires on every mousemove; `onResizeEnd(finalWidth)` fires once on mouseup (pages write localStorage). `min`/`max` pre-computed by the calling page.
...
- **h-full requirement**: These four components must keep `h-full` on their outer divs — without it, `flex-1` children collapse to 0px because the page-level wrapper is a plain block container and does not propagate stretch height automatically. (History/Images exempt: their panels are written inline on the wrapper div with no component layer.) Do not remove `h-full`.
