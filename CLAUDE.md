...
- **Context label**: When a message is sent with context attached, a `context_label` string is stored alongside it — `localContext.label` (e.g. `📄 Introduction`) for popup attachments, `'Selected text'` for editor selections. The label is stored in `chat_history` on the backend (`context_label` field on user entries) and loaded back on mount. User messages with a `context_label` display a small `📎 {label}` tag above the message bubble, right-aligned. The Rewrite button passes the heading text as `context_label` via `handleSectionRewrite(sectionContent, headingText)` → `DocumentTree` now calls `onSectionRewrite(content, h.text)`.
