...
5. **History** (`/document/:id/history`) — snapshot list left, version detail + MarkdownPreview middle, sharing & comments right. ContextBar actions: Share this version / Revoke (variant: 'default', left; conditional on share token) · Restore this version (variant: 'primary', right). All three disabled when no snapshot selected. VERSION and COMMENTS panel headers are label-only.
