...
2. **Library** (`/home`) — document list left, document detail right. ContextBar: Open · Rename · Delete when a doc is selected. Left panel actions: New Document · From template… · Import (.docx). Import triggers a hidden `<input type="file">` → `api.importDocument()` → navigates to the new document on success.
