...
**Tier 2 — Page context bar (ContextBar):** Below the global bar. Left side: tab navigation (Document / Evidence / History); active tab uses `border-b-2 border-gray-900` flush with the bar bottom (all tabs carry transparent `border-b-2` to prevent layout shift; inactive hover: `border-gray-300`). Right side: page-specific action buttons (outlined). ContextBar accepts a `tabs` prop: `[{ label, active, onClick }]`. Action objects support `disabled: true`. The optional `controls` prop renders between the tabs and the actions group. The optional `rightControls` prop renders inside the actions flex row, to the left of the action buttons — use this for dropdowns that must sit alongside action buttons. Per-page action inventories are in App Architecture below.
