...
- **Context chip**: shows the attachment label (e.g. `📄 Introduction`) or `Selected text (N chars)` for editor selections. Displays character count as `{originalLength} / 6000 chars`. If the original content exceeded 3000 chars, the chip switches to amber styling (`bg-amber-50 border-amber-200`) and shows `⚠ {N} / 6000 chars (truncated)` with a tooltip. Dismissed with ×. No emoji prefix — the chip styling makes the attachment nature clear.
